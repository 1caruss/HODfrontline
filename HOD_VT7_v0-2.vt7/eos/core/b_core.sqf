...
// SPAWN PATROLS		
	...
	for "_counter" from 1 to _PApatrols do {	
						_pos = [_mPos, _Placement, random [10,90,170]] call BIS_fnc_relPos;
...
//SPAWN LIGHT VEHICLES		
	...
	for "_counter" from 1 to _LVehGroups do {
		...
		_newpos = [_mPos, (_Placement +200), random [10,90,170]] call BIS_fnc_relPos;
...
//SPAWN ARMOURED VEHICLES
	...
	for "_counter" from 1 to _AVehGroups do {
					...
					_newpos = [_mPos, _Placement, random [10,90,170]] call BIS_fnc_relPos;
...
//SPAWN HELICOPTERS		
	...
	if ((_fSize select 0) > 0) then {_vehType=4}else{_vehType=3};
				_newpos = [(markerpos _mkr), 3000, random [10,90,170]] call BIS_fnc_relPos;	
